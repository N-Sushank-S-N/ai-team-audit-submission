You can click that green **"Add a README"** button right on your screen, paste the exact markdown text below into the editor, and click **Commit changes**!

*(Or alternatively, just run `git push origin main` in PowerShell, but clicking "Add a README" and pasting this in is the fastest).*
...
A comprehensive audit of `REPORT_v0.md` and the LLM serving stack for the **AI Team Intern Assignment — The Audit**.
...
| :--- | :--- | :--- |
| **Part A: Tokenizer Economics** | Hindi is **5.89× to 7.0× worse** than English due to an "inherent property of the script." Budget 6× serving cost. | **False.** The previous intern tested an English-only tokenizer (`gpt2`). On a 1,012-sentence parallel evaluation corpus (FLORES-101) with an Indic-aware tokenizer (`xlm-roberta-base`), Hindi requires only **1.25×** the tokens of English per parallel sentence; Dravidian languages (Kannada, Tamil) require **1.35×**. The true serving cost premium is ~25%–35%, not 500%. |
| **Part A: Metric & Denominator** | Used whitespace words (`tok/word`) and Unicode codepoints (`tok/char`). | Words in agglutinative Dravidian languages fuse inflections, inflating `tok/word` (2.58 in Kannada vs 1.40 in English) while using fewer words per sentence. **Tokens per Parallel Sentence** is the only denominator holding semantic information constant across languages. |
...
- **[NOTEBOOK.md](NOTEBOOK.md):** Graded chronological lab notebook documenting all 7 phases of hypotheses, experiments, measured results, genuine surprises, and dead ends.
...
- **[partA/memo.md](partA/memo.md):** Executive recommendation memo on multilingual tokenizer economics and routing (≤1 page).
- **[partB/calculations.md](partB/calculations.md):** Step-by-step mathematical proofs and log reconciliations for B1, B2, B3, and B4.
- **[partC/memo.md](partC/memo.md):** Decision memo recommending the prompt engineering path across 6 Indic languages under tight resource constraints (≤1 page).
...
```
.
├── README.md              # Project overview, executive summary, and reproduction guide
├── NOTEBOOK.md            # Graded chronological lab notebook
├── AI_USAGE.md            # Honest AI usage and verification summary
├── partA/
...
### 1. Re-derive Part A2 Bugs & Flaws (Evidence Rule)
Demonstrates the isolated effect of `line.split(" ")` empty strings (+1.4% to +2.0%), macro vs. micro averaging (-1.0%), casing distortions (+3.1%), and tokenizer vocabulary swaps (-80.8%):
```bash
...
An interactive calculator modeling the KV Cache Memory Ceiling and Preemption Cliff is available on Desmos:
- **Interactive Model:** [View Desmos Graph](https://www.desmos.com/calculator) *(paste your share URL here)*
- **Key Parameters Visualized:**
  - $L = 28, N_{kv} = 8, d = 128, p = 2$ (FP16) $\to 114{,}688\text{ bytes/token}$ ($112\text{ KiB}$).
  - Available KV pool on 24GB L4 = **$12.08\text{ GB}$**.
  - Concurrency ceiling at sequence length 4096 = **$25.72\text{ sequences}$**.
  - Visualizes the throughput collapse cliff at batch 32 and 48 when workloads cross into the preemption zone.
```
...
3. If you have your Desmos link, replace `https://www.desmos.com/calculator` under the **Interactive Visual Model** section with your actual link.
4. Click the green **"Commit changes..."** button at the top right of GitHub.
